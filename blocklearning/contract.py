...
class Contract:
    def __init__(self, log, provider, abi_file, account, passphrase, contract_address):
        ...
        (web3, contract, default_opts) = get_web3(
            provider, abi_file, account, passphrase, contract_address
        )
        self.web3 = web3
    ...
    def get_submissions_for_scoring(self):
        [round, trainers, submissions] = (
            self.contract.functions.getUpdatesForScore().call(self.default_opts)
        )
        return (round, trainers, submissions)

    def get_submissions_for_aggregation(self):
        [round, trainers, submissions] = (
            self.contract.functions.getUpdatesForAggregation().call(self.default_opts)
        )
        return (round, trainers, submissions)

    def get_scorings(self):
        [trainers, scorers, scores] = self.contract.functions.getScores().call(
            self.default_opts
        )
    ...
    def register_as_scorer(self):
        self.__unlock_account()
        if not self.contract.functions.registeredScorers(self.account).call(
            self.default_opts
        ):
    ...
    def register_as_aggregator(self):
        self.__unlock_account()
        if not self.contract.functions.registeredAggregators(self.account).call(
            self.default_opts
        ):
            tx = self.contract.functions.registerAggregator().transact(
                self.default_opts
            )
            return tx, self.__wait_tx(tx)
    ...
    def submit_submission(self, submission):
        self.__unlock_account()
        tx = self.contract.functions.submitUpdate(submission).transact(
            self.default_opts
        )
        return tx, self.__wait_tx(tx)
    ...
    def submit_scorings(self, trainers, scores):
        self.__unlock_account()
        tx = self.contract.functions.submitScores(trainers, scores).transact(
            self.default_opts
        )
        return tx, self.__wait_tx(tx)
    ...
    def submit_aggregation(self, weights_id):
        self.__unlock_account()
        tx = self.contract.functions.submitAggregation(weights_id).transact(
            self.default_opts
        )
        return tx, self.__wait_tx(tx)
    ...
    def submit_aggregation_with_gradients(self, weights_id, trainers, gradients_ids):
        self.__unlock_account()
        tx = self.contract.functions.submitAggregationWithGradients(
            weights_id, trainers, gradients_ids
        ).transact(self.default_opts)
        return tx, self.__wait_tx(tx)

    def confirm_backpropagation(self):
        self.__unlock_account()
        tx = self.contract.functions.confirmBackpropagation().transact(
            self.default_opts
        )
        return tx, self.__wait_tx(tx)
    ...
    def __wait_tx(self, tx):
        self.log.info(
            json.dumps({"event": "tx_start", "tx": tx.hex(), "ts": time.time_ns()})
        )
