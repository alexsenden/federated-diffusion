...
class Model:
    ...
    def train(self, x_train, y_train, x_val, y_val):
        history = self.model.fit(
            x_train, y_train, verbose=0, epochs=1, validation_data=(x_val, y_val)
        )

        return history.history

    def evaluate(self, x_val, y_val):
        return self.model.evaluate(x=x_val, y=y_val, verbose=0, return_dict=True)
    ...
    def set_weights(self, serialized):
        if len(serialized) != self.count:
            raise Exception(
                f"Wrong number of serialized weights. Expected ${self.count}, got ${len(serialized)}"
            )
